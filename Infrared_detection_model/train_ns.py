...
parser.add_argument("--dataset_names", default=['mo-ns7nmf'], type=list)
...
parser.add_argument("--noise_img_dir", default=r'/home/data/lyh/SCTransNet-main7/nosie7', type=str, help="Directory containing noise images")
...
def add_noise(img, noise_images, m=0.5):
    # 从噪声图像列表中随机选择一张
    noise_img = random.choice(noise_images)
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        for idx_iter, (img, gt_mask) in enumerate(train_loader):
            ...
            # 计算损失并更新总损失
            loss_mse = nn.MSELoss()(preds[-1], preds_noise[-1])
            #loss_mse = self_information_loss(preds[-1], preds_noise[-1])
            
            

            # 计算总损失：原损失 + 自信息损失
            loss = net.loss(preds, gt_mask) + loss_mse
